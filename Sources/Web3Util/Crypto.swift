...
class Crypto {
    ...
    func aes256CMac(data: [UInt8], key: [UInt8]) -> [UInt8] {
        let result = aes256Enc(data: data, iv: [UInt8](repeating: 0, count: 16), key: key).suffix(16)
        return Array(result)
    }
    ...
    func secp256k1PublicFromPrivate(_ privKey: [UInt8], _ compressed: Bool = false) -> [UInt8] {
        var pubKey = secp256k1_pubkey()
        _ = secp256k1_ec_pubkey_create(secp256k1Ctx, &pubKey, privKey)
        return _secp256k1PubToBytes(&pubKey, compressed: compressed)
    }

    func secp256k1RecoverPublic(r: [UInt8], s: [UInt8], recId: UInt8, hash: [UInt8], compressed: Bool = false) -> [UInt8] {
    ...
    private func _secp256k1PubToBytes(_ pubKey: inout secp256k1_pubkey, compressed: Bool) -> [UInt8] {
        let len = compressed ? 33 : 65
        var pubKeyBytes = [UInt8](repeating: 0, count: len)
        var outputLen = len
        let size = compressed ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED
        _ = secp256k1_ec_pubkey_serialize(secp256k1Ctx, &pubKeyBytes, &outputLen, &pubKey, UInt32(size))
